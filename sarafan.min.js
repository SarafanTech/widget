...
if(test){console.log('search responses start');console.log(responses);console.log('search responses finish');}if(responses.result==='Error'){return false;}responses.forEach(function(response,i){if(response&&response.result.toLowerCase()==='ok'){sarafanSDK.cache[response.image]=response.offers.slice(0);for(var _i=0;_i<response.offers.length;_i++){var offer=response.offers[_i];if(!sarafanSDK.products.has(offer.product_id)){sarafanSDK.products.set(offer.product_id,response.image);}}sarafanSDK.commonInfoCache[response.image]={'mediaId':response['media_id'],'mediaLinkId':response['media_link_id']};if(response.ad){sarafanSDK.ad[response.image]=response.ad.slice(0);}if(response.pins){sarafanSDK.pins[response.image]=response.pins.slice(0);for(var _i2=0;_i2<response.pins.length;_i2++){for(var j=0;j<response.pins[_i2].offers.length;j++){var _offer=response.pins[_i2].offers[j];if(!sarafanSDK.products.has(_offer.product_id)){sarafanSDK.products.set(_offer.product_id,response.image);}}}}if(response.tags){sarafanSDK.mediaTags[response.image]=response.tags.slice(0);}if(Object.keys(response.offers).length>0){sarafanSDK.cacheQueueStatus=true;}if(window.location.host==="www.wday.ru"&&notLoadInFloorUrls){var find=false;for(var _i3=0;_i3<notLoadInFloorUrls.length;_i3++){if(notLoadInFloorUrls[_i3]===response.image){find=true;break;}}if(!find){sarafanSDK.cacheQueue[response.image]=response.offers.slice(0);}}else{sarafanSDK.cacheQueue[response.image]=response.offers.slice(0);}sarafanSDK.floorQueue[response.image]=response.offers.slice(0);}});if(test){console.log('cache start');console.log(sarafanSDK.cache);console.log('cache finish');}return sarafanSDK.cache;});};Sarafan.prototype.view=function(src,mode,version){var requestStr=baseApiUrl+'view?image='+encodeURIComponent(src)+'&mode='+encodeURIComponent(mode);if(version){requestStr=requestStr+'&version='+encodeURIComponent(version);}fetch(requestStr,{credentials:"include"});};/*
    ...
    */Sarafan.prototype.options=function(version){//this.pageSessionId = this.guid();
...
if(hostname.indexOf('wmj.ru')+1){return'wmj';}return'other';}})();;(function(){var _marked=/*#__PURE__*/regeneratorRuntime.mark(returnSlide);function checkNewSlider(){if(location.host==="www.wday.ru"||location.host==="widget.local"||location.host==="www.woman.ru"||location.host==="www.marieclaire.ru"){return true;}else{return false;}}var WIDGET_TYPE_FLOOR="floor";var WIDGET_TYPE_VIEW="view";var WIDGET_TYPE_BUTTON="button";var WIDGET_TYPE_PIN="pin";var isNewSlider=checkNewSlider();// can be dev or number
var version=window.sarafanSDK.version,baseApiUrl=window.sarafanSDK.baseApiUrl;var test=window.sarafanSDK.test;var observer;var googletag;var sliders=[];var MULTIPLE_MODE="multiple";var VIDEO_MODE="video";var SINGLE_MODE="single";var targetNode=document.body;var observerConfig={//attributes: true,
...
_showPrice=false,_enableButtonMode=true,_enableViewMode=true,_enableDblClick=false,_dblClickCampaign='',_enableFloorMode=false,_enableLazyLoad=true,_enableFrames=false,_language='en',_processImageObject=false,_offersPanelOffsetY=100,// default 100%. Если 0%, то панель офферов возникает по центру кнопки,
...
_showFirstPin=true,offersFloor=[],optionsHandlers={'default-theme':function defaultTheme(){_theme='default-theme';},'cosmo-theme':function cosmoTheme(){_theme='cosmo-theme';},'7days-theme':function daysTheme(){_theme='7days-theme';},'fivmagazine-theme':function fivmagazineTheme(){_theme='fivmagazine-theme';},'pink-theme':function pinkTheme(){_theme='pink-theme';},'brown-theme':function brownTheme(){_theme='brown-theme';},'orange-theme':function orangeTheme(){_theme='orange-theme';},button:function button(){_pageMode='button';},inpicture:function inpicture(){_pageMode='inpicture';},view:function view(){_pageMode='view';},medium:function medium(){_buttonSize='';},small:function small(){_buttonSize='small';},large:function large(){_buttonSize='large';},topleft:function topleft(){_corner='topleft';},topright:function topright(){_corner='topright';},bottomleft:function bottomleft(){_corner='bottomleft';},bottomright:function bottomright(){_corner='bottomright';},bottomcenter:function bottomcenter(){_corner='bottomcenter';},'btn-standart':function btnStandart(){_btnType='btn-standart';},'btn-only-icon':function btnOnlyIcon(){_btnType='btn-only-icon';},'btn-only-text':function btnOnlyText(){_btnType='btn-only-text';},'all-classes':function allClasses(){_imagesSearchType='all-classes';},'sarafan-class':function sarafanClass(){_imagesSearchType='sarafan-class';},'load-view-simply':function loadViewSimply(){_viewModeType='load-view-simply';},'load-view-all':function loadViewAll(){_viewModeType='load-view-all';},'view-mode-probability':function viewModeProbability(percent){_viewModeProbability=percent;}};if(/https:\/\/www.wday.ru\/moda-shopping\/novosty\/kak-novyiy-stilist-keyt-middlton-izmenila-ee-garderob/.test(location.href)||/https:\/\/www.wday.ru\/moda-shopping\/gar\/bazovyi-letnii-garderob-dlya-devushek-plus-size/.test(location.href)||/https:\/\/www.wday.ru\/moda-shopping\/style\/katya-gershuni-nosite-telnyashki-i-hudeyte-chtobyi-vyiglyadet-na-10-let-moloje/.test(location.href)){_dblClickCampaign='81006599/hmiru-wday/sarafan';}if(/http:\/\/www.woman.ru\/fashion\/celebrities\/article\/229877/.test(location.href)){_dblClickCampaign="/81006599/hmiru-woman/sarafan";}var options=me.getAttribute('data-sarafan-options')||"";options=options.toLowerCase().split(',').map(function(value){return value.trim();});options.forEach(function(option){if(optionsHandlers[option]){optionsHandlers[option]();}});var optionHandlers2={'theme':function theme(val){_theme=val;},'border':function border(val){_border=val;},'pageMode':function pageMode(val){_pageMode=val;},'buttonSize':function buttonSize(val){_buttonSize=val;},'corner':function corner(val){_corner=val;},'btnType':function btnType(val){_btnType=val;},'imagesSearchType':function imagesSearchType(val){_imagesSearchType=val;},'viewModeType':function viewModeType(val){_viewModeType=val;},'viewModeProbability':function viewModeProbability(val){_viewModeProbability=val;},'showPrice':function showPrice(val){_showPrice=val;},'onHover':function onHover(val){_onHover=val;},'enableButtonMode':function enableButtonMode(val){_enableButtonMode=val;},'enableViewMode':function enableViewMode(val){_enableViewMode=val;},'enableFloorMode':function enableFloorMode(val){_enableFloorMode=val;},'enableFrames':function enableFrames(val){_enableFrames=val;},'enableDblClick':function enableDblClick(val){_enableDblClick=val;},'dblClickCampaign':function dblClickCampaign(val){_dblClickCampaign=val;if(/https:\/\/www.wday.ru\/moda-shopping\/novosty\/kak-novyiy-stilist-keyt-middlton-izmenila-ee-garderob/.test(location.href)||/https:\/\/www.wday.ru\/moda-shopping\/gar\/bazovyi-letnii-garderob-dlya-devushek-plus-size/.test(location.href)||/https:\/\/www.wday.ru\/moda-shopping\/style\/katya-gershuni-nosite-telnyashki-i-hudeyte-chtobyi-vyiglyadet-na-10-let-moloje/.test(location.href)){_dblClickCampaign='81006599/hmiru-wday/sarafan';}if(/http:\/\/www.woman.ru\/fashion\/celebrities\/article\/229877/.test(location.href)){_dblClickCampaign="/81006599/hmiru-woman/sarafan";}},'enableLazyLoad':function enableLazyLoad(val){_enableLazyLoad=val;},'language':function language(val){_language=val;},'floorWrapperSubTitle':function floorWrapperSubTitle(val){_floorWrapperSubTitle=val;},'processImageObject':function processImageObject(val){_processImageObject=val;},'showFirstPin':function showFirstPin(val){_showFirstPin=val;},'offersPanelOffsetY':function offersPanelOffsetY(val){_offersPanelOffsetY=val;},'btnShiftX':function btnShiftX(val){_btnShiftX=val;},'btnShiftY':function btnShiftY(val){_btnShiftY=val;}};if(parseInt(me.getAttribute('data-sarafan-view-probability'))){_viewModeProbability=parseInt(me.getAttribute('data-sarafan-view-probability'));}var button,offers,offersShift=10,buttonClass='sarafan-button',buttonText=window.sarafanSDK.site==='cosmo'?'Купить такой образ':'Купить этот образ',additionalOfferText='Дополни образ',viewsTitleText=null,_floorWrapperSubTitle=null,floorWrapperDescription=null,// empty default
offerTemplate,svgEye,svgEyeSmall,iconCosmo,buttonTemplate,offersPanelTemplate,offersPanelTemplate2,modeButtonClass='sarafan-button-mode',modeViewClass='sarafan-view-mode',modeInPictureClass='sarafan-inpicture',emptyWrapperClass='sarafan-empty-div',sarafanViewPanelClass='sarafan-offers-view-panel',sarafanViewPanelClass2='sarafan-offers-view-panel2';sarafanButtonPanelClass="sarafan-offers-button-panel",sarafanPinPanelClass="sarafan-offers-pin-panel",sarafanFloorPanelClass='sarafan-offers-floor-panel',floorClass='sarafan-floor',sarafanInPicturePanelClass='sarafan-inpic-panel',multipleOffersPanelClass='sarafan-offers-panel2-multiple',singleOfferPanelClass='sarafan-offers-panel2-single',minWidthOfImageForDisplayBottomOffersWrapper=200,minHeightOfImageForDisplayBottomOffersWrapper=300,minWidthForInPicture=320,minWidthOfImageForChangeRow=500,nextFloor=null,nextFloorOffset=5000,populateNextFloorLocked=false;if(isNewSlider){sarafanOffersPanelClass='sarafan-offers-panel2';}else{sarafanOffersPanelClass='sarafan-offers-panel';}var currencies={RUR:'P',RUB:'P',USD:'$'};/*
...
sarafanSDK.sendEvent(_objectSpread({},dataForStat,{offersData:nextSlideData.value.offersData,type:"viewport.offers"}));}place.classList.add('sarafan-first-showed');}}};if(!dataForStat.mode||dataForStat.mode!=='floor'||buildFirstFloor){buildSlide('first');}var slider=generateSliderTNS(offersNode,offersPanel);if(dataForStat.mode&&dataForStat.mode==='floor'&&!buildFirstFloor){offersPanel.querySelector('.sarafan-tns-controls').removeAttribute('hidden');}offersPanel.querySelector('button[data-controls="next"]').onclick=function(){var sliderInfo=slider.getInfo();if(sliderInfo.slideItems[sliderInfo.index].nextElementSibling&&sliderInfo.slideItems[sliderInfo.index].nextElementSibling.classList.contains('sarafan-empty')){buildSlide('next');if(_enableDblClick){dblClickQueue.forEach(function(v){dblClickRenderSlot(v.div,v.str,v.placement,v.resolutions,v.id,200,slider,slicedOffers);});var _sarafanOffers=offers.querySelector('.sarafan-offers');var dblClicks=_sarafanOffers.querySelectorAll('sfandiv.offer > div');dblClickQueue=[];}}};return slider;}/* generate one slide with sliced element */function generateSarafanSlide(e,dataForStat){var sarafanSlide=document.createElement('sfandiv');sarafanSlide.classList.add('sf-slide-content');var _loop=function _loop(i){var offerADV=null;// NODE object of offer for statistics working
...
var _imageWrapper=image.closest("."+emptyWrapperClass);if(location.host==="www.wday.ru"){var slide=image.closest(".swiper-zoom-container");if(slide){_imageWrapper=slide;}}if(location.host==="www.woman.ru"){var _slide=image.closest(".gallery__slide");if(_slide){_imageWrapper=_slide;}}if(_imageWrapper){}else{_imageWrapper=document.createElement('sfandiv');_imageWrapper.classList.add(emptyWrapperClass);var imageStyles=window.getComputedStyle(image);_imageWrapper.style.width=image.style.width?image.style.width:imageStyles.width;if(window.location.hostname=='arcor.de'||window.location.hostname=='www.arcor.de'){var pWrap=image.parentNode;_imageWrapper.style.width=window.getComputedStyle(pWrap).width;}if(window.sarafanSDK.site!=='wmj'&&location.hostname!=="www.ellegirl.ru"&&location.hostname!=="www.woman.ru"&&location.hostname!=="www.spb.kp.ru"&&location.hostname!=="m.advert.dev.kp.ru"){_imageWrapper.style.height='100%';}// if (location.hostname === "www.woman.ru" && isMobile()) {
...
button.style.left=_btnShiftX+'px';button.style.top=_btnShiftY+'px';var cartImgBtn=_imageWrapper.getElementsByClassName("sarafan-image__btn-container")[0]||_imageWrapper.getElementsByClassName("sarafan-image-btn")[0];if(_corner==='topleft'){button.style.left=_btnShiftX+'px';button.style.top=_btnShiftY+'px';button.style.bottom=button.style.right='initial';}else if(_corner==='topright'){button.style.right=_btnShiftX+'px';button.style.top=_btnShiftY+'px';button.style.left=button.style.bottom='initial';}else if(_corner==='bottomleft'){if(cartImgBtn&&cartImgBtn.dataset.corner===_corner&&window.getComputedStyle(button).position==="absolute"){var btnsContainers=getBtnsContainer(_corner);clearOffset(button);clearOffset(cartImgBtn);button.style.bottom=_btnShiftY+'px';cartImgBtn.style.bottom=_btnShiftY+'px';button.style.marginLeft=_btnShiftY+'px';cartImgBtn.style.marginLeft=_btnShiftY+'px';button.style.bottom=_btnShiftY+'px';btnsContainers.appendChild(cartImgBtn);btnsContainers.appendChild(button);_imageWrapper.appendChild(btnsContainers);}else{button.style.bottom=_btnShiftY+'px';button.style.left=_btnShiftX+'px';button.style.top=button.style.right='initial';}}else if(_corner==='bottomright'){button.style.right=_btnShiftX+'px';button.style.bottom=_btnShiftY+'px';button.style.left=button.style.top='initial';}else if(_corner==='bottomcenter'){if(cartImgBtn&&cartImgBtn.dataset.corner===_corner&&window.getComputedStyle(button).position==="absolute"){clearOffset(button);clearOffset(cartImgBtn);var _btnsContainers=getBtnsContainer(_corner);button.style.bottom=_btnShiftY+'px';_btnsContainers.appendChild(cartImgBtn);_btnsContainers.appendChild(button);if(location.host==="www.wday.ru"&&_imageWrapper.classList.contains("swiper-zoom-container")){var panel=document.querySelector(".fullscreen-slider__bottom-panel");var offset;panel?offset=panel.offsetHeight-3:offset=45;offset=Math.max(offset,45);cartImgBtn.style.bottom=offset+"px";button.style.bottom=offset+_btnShiftY+"px";}if(location.host==="www.woman.ru"){var _offset=0;var gallery=_imageWrapper.closest(".gallery");if(gallery){var pager=gallery.getElementsByClassName("gallery-pager")[0];if(pager&&window.getComputedStyle(pager).position==="absolute"){_offset=pager.offsetHeight;}if(isMobile()){_offset=image.getBoundingClientRect().bottom-document.documentElement.clientHeight*0.89;if(_offset<0){_offset=0;}}}cartImgBtn.style.bottom=_offset+"px";button.style.bottom=_offset+_btnShiftY+"px";}_imageWrapper.appendChild(_btnsContainers);}else{var btnWidthRect=button.getBoundingClientRect();var btnWidth=btnWidthRect.right-btnWidthRect.left;button.style.left='calc(50% - '+btnWidth/2+'px)';button.style.bottom=_btnShiftY+'px';button.style.top=button.style.right='initial';button.classList.add('sfn-bottomcenter');if(location.host==="www.gala.de"){button.style.left='calc(50% - '+64+'px)';}if(location.host==="www.wday.ru"&&_imageWrapper.classList.contains("swiper-zoom-container")){var _panel=document.querySelector(".fullscreen-slider__bottom-panel");var _offset2;_panel?_offset2=_panel.offsetHeight-3:_offset2=45;_offset2=Math.max(_offset2,45);button.style.bottom=_offset2+_btnShiftY+"px";}if(location.host==="www.woman.ru"){var _offset3=0;var _gallery=_imageWrapper.closest(".gallery");if(_gallery){var _pager=_gallery.getElementsByClassName("gallery-pager")[0];if(_pager&&window.getComputedStyle(_pager).position==="absolute"){_offset3=_pager.offsetHeight;}if(isMobile()){_offset3=image.getBoundingClientRect().bottom-document.documentElement.clientHeight*0.89;if(_offset3<0){_offset3=0;}}}button.style.bottom=_offset3+_btnShiftY+"px";}}}else if(_corner==='marginbottom'){button.style.position='initial';button.style.display='block';button.classList.add('sfn-marginbottom');// var btnWidthRect = button.getBoundingClientRect();
...
if(test)console.log('_____________________________________________________ОФФЕРЫ В INPICTURE ДО ПОМЕЩЕНИЯ В КАРУСЕЛЬ____________________');if(test)console.log(sarafanOffers);// sarafanOffers.innerHTML += "";
var slider=generateSliderNode(sliced,sarafanOffers,offers);// make listener for legacy information opener (button mode)
var legalOpener=offers.querySelector('.sarafan-offers-panel-footer');if(legalOpener){// legalOpener.addEventListener("click", () => { toggleLegacyOpened('.' + sarafanOffersPanelClass, 'legacy-opened'); event.stopPropagation(); });
legalOpener.addEventListener("click",function(){toggleLegacyOpened('.'+sarafanOffersPanelClass,'legacy-opened');checkCartIsClicked(event);event.stopPropagation();});}}/* generate block with offers and ads in pin */function showSarafanPinOffers(imageSrc,pinsOffers,pin,yPos,imageCoords,imageWidth,image,imageData){offers=document.querySelector(".".concat(sarafanButtonPanelClass,"[data-sarafan-src=\"").concat(imageSrc,"\"]"));hideAllPanels();if(test)console.log('Sarafan widget: view action on image '+imageSrc);window.sarafanSDK.view(imageSrc,'button',version);if(!offers){offers=document.createElement('sfandiv');offers.id=sarafanOffersPanelClass;offers.classList.add(sarafanOffersPanelClass);if(_theme){offers.classList.add('theme-'+_theme);}if(isNewSlider){offers.innerHTML=offersPanelTemplate2;}else{offers.innerHTML=offersPanelTemplate;}offers.setAttribute('data-sarafan-src',imageSrc);offers.addEventListener('click',function(event){checkCartIsClicked(event);event.stopPropagation();// if (test) console.log('stop propogation');
});}else{if(isNewSlider){offers.innerHTML=offersPanelTemplate2;}else{offers.innerHTML=offersPanelTemplate;}offers.classList.remove('legacy-opened');offers.setAttribute('data-sarafan-src',imageSrc);}offers.classList.add(sarafanPinPanelClass);document.querySelector('body').appendChild(offers);offers.insertAdjacentHTML('afterbegin','<div class="close-area" onclick="{'+'this.parentNode.style.display=\'none\'; '+'let buttonsForGlamour = document.getElementsByClassName(\'pin-div-glamour\');'+'        for (let i = 0; i< buttonsForGlamour.length; i++) {'+'            buttonsForGlamour[i].classList.remove(\'pin-div-glamour-active\');'+'        }}"><div class="close-btn">'+'<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" x="0px" y="0px" viewBox="0 0 32.526 32.526" xml:space="preserve" width="9px" height="9px">'+'<polygon points="32.526,2.828 29.698,0 16.263,13.435 2.828,0 0,2.828 13.435,16.263 0,29.698 2.828,32.526 16.263,19.091   29.698,32.526 32.526,29.698 19.091,16.263 " fill="#FFFFFF"/>'+'</div></div>');var sarafanOffers=offers.querySelector('.sarafan-offers');var adOffersArr=window.sarafanSDK.getAdOffersForPicture(imageSrc),elementsCounter=1;var offersArr=pinsOffers;var totalArr=[],sliced=[];if(offersArr.length>=4){elementsCounter=3;}else if(offersArr.length===3){elementsCounter=3;}else if(offersArr.length===2){elementsCounter=2;}else if(offersArr.length===1){}var mode;if(isNewSlider){if(_enableDblClick){totalArr=getOfferWithDblClick(offersArr);}else{totalArr=getOffersWithAdfox(offersArr,window.sarafanSDK.getAdOffersForPicture(imageSrc));}setOffersPanelWidthByOffersCount(offers,offersArr);if(offersArr.length===1){mode=SINGLE_MODE;offers.classList.add(singleOfferPanelClass);}else{mode=MULTIPLE_MODE;offers.classList.add(multipleOffersPanelClass);}}else{/* for mobile versions, when resolution less then mobileMinWidth 'px' slider shows 2 offers */if(checkMobileRes())elementsCounter=1;/* get complete the look cards */var completes=adOffersArr.filter(function(e){return e.type==='A';});/* get micromedia cards */var micromedia=adOffersArr.filter(function(e){return e.type==='adfox_mm';});/* get adfox cards */var ads=adOffersArr.filter(function(e){return!completes.includes(e);}).filter(function(e){return e.type==='adfox';}).filter(function(e){return e.insert_code_div.length>5;});/* get promo cards */var promo=adOffersArr.filter(function(e){return e.type==='adfox_p';});/* get offers count for current resolution */var cnt=getOffersCnt(document.documentElement.clientWidth);/* Merge arrays with offers, ADs and complete the look's at the end */if(typeof window.Ya!=='undefined'&&typeof window.Ya.adfoxCode!=='undefined'){micromedia=micromedia.concat(ads);}totalArr=insertAdOffers(offersArr,micromedia,cnt-1).concat(completes);if(totalArr.length===1||totalArr.length===2&&cnt>2){offers.classList.remove('sf-4');offers.classList.remove('sf-3');offers.classList.add('sf-2');}else if(totalArr.length===3&&cnt>3){offers.classList.remove('sf-2');offers.classList.remove('sf-4');offers.classList.add('sf-3');}else{switch(cnt){case 1:case 2:{offers.classList.remove('sf-4');offers.classList.remove('sf-3');offers.classList.add('sf-2');}break;case 3:{offers.classList.remove('sf-2');offers.classList.remove('sf-4');offers.classList.add('sf-3');}break;default:{offers.classList.remove('sf-2');offers.classList.remove('sf-3');offers.classList.add('sf-4');}break;}}/* Split offers by elements count */sliced=splitArray(totalArr,cnt);if(promo.length>0)sliced.splice(1,0,promo);}var pinCoordinates=getCoords(pin);var clientWidth=window.innerWidth||document.documentElement.clientWidth;var clientHeight=window.innerHeight||document.documentElement.clientHeight;offers.style.display='inline-block';offers.classList.add('count-'+totalArr.length);offers.dataset.countOffers=totalArr.length;// initial position for initial rendering
...
if(window.sarafanSDK.site==='eva'){fixTop=bodyOffsets.top;fixLeft=bodyOffsets.left;}var yOffset=0;if(yPos>70){yOffset=offers.offsetHeight+60;}offers.style.top=pinCoordinates.top-yOffset+50+'px';offers.style.left=pinCoordinates.left+'px';checkElementPositionForViewport(offers,clientWidth,clientHeight);if(test)console.log('_____________________________________________________ОФФЕРЫ В КНОПКЕ ДО ПОМЕЩЕНИЯ В КАРУСЕЛЬ____________________');if(test)console.log(sarafanOffers);if(isNewSlider){var slider=new Slider(sarafanOffers,mode,WIDGET_TYPE_PIN,_objectSpread({media:image,tags:sarafanSDK.getTagsForPicture(imageSrc),component:offers},sarafanSDK.commonInfoCache[imageSrc],imageData,{mode:'pin'}));sliders.push(slider);slider.appendOffers(totalArr);}else{var _slider2=generateSliderNode(sliced,sarafanOffers,offers,_objectSpread({media:image,tags:sarafanSDK.getTagsForPicture(imageSrc),component:offers},sarafanSDK.commonInfoCache[imageSrc],imageData,{mode:'pin'}));}pin.classList.add('pin-div-glamour-active');// make listener for legacy information opener (button mode)
var legalOpener=offers.querySelector('.sarafan-offers-panel-footer');if(legalOpener){// legalOpener.addEventListener("click", () => { toggleLegacyOpened('.' + sarafanOffersPanelClass, 'legacy-opened'); event.stopPropagation(); });
...
_imageWrapper2=image.closest("."+emptyWrapperClass);if(!_imageWrapper2){_imageWrapper2=document.createElement('sfandiv');_imageWrapper2.classList.add(emptyWrapperClass);var imageStyles=window.getComputedStyle(image);_imageWrapper2.style.height='100%';_imageWrapper2.style.width=imageStyles.width;_imageWrapper2.style.margin=imageStyles.margin;wrap(image,_imageWrapper2);}_imageWrapper2.appendChild(widgetWrapper);}if(_theme){if(_imageWrapper2)_imageWrapper2.classList.add('theme-'+_theme);offers.classList.add('theme-'+_theme);}offers.addEventListener('click',function(event){checkCartIsClicked(event);event.stopPropagation();if(test)console.log('stop propogation');});}var offersArr=window.sarafanSDK.getOffersForPicture(dataImageSrc),adOffersArr=window.sarafanSDK.getAdOffersForPicture(dataImageSrc),elementsCounter=3;offers.dataset.countOffers=offersArr.length;var sarafanOffers=offers.querySelector('.sarafan-offers');var totalArr;var sliced=[];/* for mobile versions, when resolution less then mobileMinWidth 'px' slider shows 2 offers */if(isNewSlider){offers.classList.add(sarafanViewPanelClass2);if(_enableDblClick){totalArr=getOfferWithDblClick(offersArr);}else{totalArr=getOffersWithAdfox(offersArr,window.sarafanSDK.getAdOffersForPicture(dataImageSrc));}offers.classList.add(multipleOffersPanelClass);var slider=new Slider(sarafanOffers,MULTIPLE_MODE,WIDGET_TYPE_VIEW,_objectSpread({media:image,tags:sarafanSDK.getTagsForPicture(dataImageSrc),component:offers},sarafanSDK.commonInfoCache[dataImageSrc],imageData,{mode:'view'}));sliders.push(slider);slider.appendOffers(totalArr);}else{if(checkMobileRes())elementsCounter=1;/* get complete the look cards */var completes=adOffersArr.filter(function(e){return e.type==='A';});/* get micromedia cards */var micromedia=adOffersArr.filter(function(e){return e.type==='adfox_mm';});/* get adfox cards */var ads=adOffersArr.filter(function(e){return!completes.includes(e);}).filter(function(e){return e.type==='adfox';}).filter(function(e){return e.insert_code_div.length>5;});/* get promo cards */var promo=adOffersArr.filter(function(e){return e.type==='adfox_p';});var imgWidth=window.getComputedStyle(image).width,wrapperWidth=window.getComputedStyle(widgetWrapper).width;/* get offers count for current resolution */var cnt=getOffersCnt(parseInt(wrapperWidth));/* Merge arrays with offers, ADs and complete the look's at the end */if(typeof window.Ya!=='undefined'&&typeof window.Ya.adfoxCode!=='undefined'){micromedia=micromedia.concat(ads);}totalArr=insertAdOffers(offersArr,micromedia.concat(ads),cnt-1).concat(completes);/* Split offers by elements count */sliced=splitArray(totalArr,cnt);if(promo.length>0)sliced.splice(1,0,promo);offers.style.width=imgWidth-2;var _slider3=generateSliderNode(sliced,sarafanOffers,offers,_objectSpread({media:image,tags:sarafanSDK.getTagsForPicture(dataImageSrc),component:offers},sarafanSDK.commonInfoCache[dataImageSrc],imageData,{mode:'view'}));}// make listener for legacy information opener (view mode)
...
}floorWrapper.dataset.countOffers=countOffers;/* for mobile versions, when resolution less then mobileMinWidth 'px' slider shows 2 offers */if(checkMobileRes())elementsCounter=1;/* get offers count for current resolution */micromedias=uniqueByProperty(micromedias,'url');adBanners=uniqueByProperty(adBanners,'insert_code_div');totalOffers=uniqueByProperty(totalOffers,'product_id');var promo=[];/* Merge arrays with offers, ADs and complete the look's at the end */if(typeof window.Ya!=='undefined'&&typeof window.Ya.adfoxCode!=='undefined'){micromedias=micromedias.concat(adBanners);}var totalArr,splice=[];if(isNewSlider){floorWrapper.classList.add(sarafanViewPanelClass2);if(_enableDblClick){totalArr=getOfferWithDblClick(totalOffers);}else{totalArr=getCollectedOffersWithAdfox(totalOffers,micromedias,promos[0]);}var s=getSlider(location.href,WIDGET_TYPE_FLOOR);if(s){s.appendOffers(totalArr);}else{var slider=new Slider(_sarafanOffers2,MULTIPLE_MODE,WIDGET_TYPE_FLOOR,{mode:"floor",component:floorWrapper,widgetId:sarafanSDK.lastWidgetId});sliders.push(slider);slider.appendOffers(totalArr);}}else{var cnt=getOffersCnt(document.documentElement.clientWidth);if(_enableDblClick){var item={type:'doubleclick',place:'ad'};totalArr=insertAfterEach(totalOffers,item,cnt-1);console.log(totalArr);promo=Object.assign({},item);promo.place='promo';if(totalArr.length<3){promo.size='sm';}else{if(cnt<3){promo.size='sm';}}}else{totalArr=insertAdOffers(uniqueByProperty(totalOffers,'buyurl'),micromedias,cnt-1);}if(totalArr.length===1||totalArr.length===2&&cnt>2){floorWrapper.classList.remove('sf-floor-4');floorWrapper.classList.remove('sf-floor-3');floorWrapper.classList.add('sf-floor-2');}else if(totalArr.length===3&&cnt>3){floorWrapper.classList.remove('sf-floor-2');floorWrapper.classList.remove('sf-floor-4');floorWrapper.classList.add('sf-floor-3');}else{switch(cnt){case 1:case 2:{floorWrapper.classList.remove('sf-floor-4');floorWrapper.classList.remove('sf-floor-3');floorWrapper.classList.add('sf-floor-2');}break;case 3:{floorWrapper.classList.remove('sf-floor-2');floorWrapper.classList.remove('sf-floor-4');floorWrapper.classList.add('sf-floor-3');}break;default:{floorWrapper.classList.remove('sf-floor-2');floorWrapper.classList.remove('sf-floor-3');floorWrapper.classList.add('sf-floor-4');}break;}}/* Split offers by elements count */splice=splitArray(totalArr,cnt);if(_enableDblClick){if(promo)splice.splice(1,0,[promo]);// if (promo.length > 0) sliced.splice(1, 0, promo);
if(promo.length>0)sliced.splice(1,0,promo);if(splice[0].length===1){splice[0].push({type:'doubleclick',place:'ad'});}}var _slider4=generateSliderNode(splice,_sarafanOffers2,floorWrapper,{mode:"floor",component:floorWrapper,widgetId:sarafanSDK.lastWidgetId});}if(!findFloor){sarafanSDK.getIncrementedWidgetId();}// make listener for legacy information opener (floor mode)
...
if(imageData['mode']==='button'||imageData['mode']==='inpicture'){showSarafanButton(image,imageData);}else if(imageData['mode']==='view'){showSarafanViewBlock(image,imageData);}};if(image.complete){if(test)console.log('картинка '+dataImageSrc+' была загружена заранее');//for lazy load in elle, brigitte
if(/lazy/.test(image.className)&&image.clientWidth<=50&&image.naturalWidth<=50){image.addEventListener('load',listener,false);}else{listener();}}else if(_processImageObject&&image.hasAttribute('itemtype')){listener();}else{if(test)console.log('добавляю событие load для картинки '+dataImageSrc);image.addEventListener('load',listener,false);}});if(location.hostname==="www.ellegirl.ru"){var fBtn=document.querySelector('.swiper-slide-active .sarafan-button');if(fBtn){var src=fBtn.getAttribute('data-sarafan-src');if(src){setTimeout(function(){var offers=document.querySelector(".".concat(sarafanFloorPanelClass,"[data-sarafan-src=\"").concat(src,"\"]"));if(!offers){fBtn.click();}},1500);setTimeout(function(){var offers=document.querySelector(".".concat(sarafanOffersPanelClass,"[data-sarafan-src=\"").concat(src,"\"]"));if(offers&&offers.style.display!=='none'){fBtn.click();}},6500);}}}if(location.hostname==="www.woman.ru"){var _fBtn=document.querySelector('.sarafan-button');if(_fBtn){var _src=_fBtn.getAttribute('data-sarafan-src');if(_src){var img=_fBtn.closest(".sarafan-empty-div").querySelector("img");if(img&&!img.matches(".gallery img")){setTimeout(function(){var offers=document.querySelector(".".concat(sarafanOffersPanelClass,"[data-sarafan-src=\"").concat(_src,"\"]"));if(!offers){_fBtn.click();}},100);setTimeout(function(){var offers=document.querySelector(".".concat(sarafanOffersPanelClass,"[data-sarafan-src=\"").concat(_src,"\"]"));if(offers&&offers.style.display!=='none'){_fBtn.click();}},5100);}}}}if(location.hostname==="www.wday.ru"){var _fBtn2=document.querySelector('.sarafan-button');if(_fBtn2){if(_fBtn2.matches(".swiper-wrapper .sarafan-button")){_fBtn2=document.querySelectorAll('.sarafan-button')[1];}var _src2=_fBtn2.getAttribute('data-sarafan-src');if(_src2){setTimeout(function(){var offers=document.querySelector(".".concat(sarafanOffersPanelClass,"[data-sarafan-src=\"").concat(_src2,"\"]"));if(!offers){_fBtn2.click();}},100);setTimeout(function(){var offers=document.querySelector(".".concat(sarafanOffersPanelClass,"[data-sarafan-src=\"").concat(_src2,"\"]"));if(offers&&offers.style.display!=='none'){_fBtn2.click();}},5100);}}}// working for floor mode. getting nextFloor
...
if(e.size==='sm'){resolutions=[280,280];}}div.insertAdjacentHTML('afterbegin','<div id=\'div-gpt-ad-'+id+'-0\'></div>');return{div:div,str:_dblClickCampaign,placement:placement,resolutions:resolutions,id:id};}function collapse(v,slider){// slider.destroy();
var c=1;var s=v.parentNode.parentNode.children.length===1?v.parentNode.parentNode:v.parentNode;s.remove();if(c>0){if(c>1){offers.classList.remove('sf-4');offers.classList.remove('sf-3');offers.classList.add('sf-2');}slider.rebuild();}sarafanOffers.style.visibility='visible';offers.querySelector('.loading1').remove();offers.querySelector('.sarafan-tns-controls').style.visibility='visible';}function dblClickRenderSlot(div,str,placement,resolutions,id,timeout,slider,slicedOffers){if(str&&placement&&resolutions&&id){setTimeout(function(){var googletag=window.googletag;var slot=googletag.defineSlot(str+'/sarafan-'+placement,resolutions,'div-gpt-ad-'+id+'-0').addService(googletag.pubads());slot.div=div;googletag.pubads().collapseEmptyDivs();googletag.pubads().addEventListener('slotRenderEnded',function(e){if(e.slot===slot){var id=e.slot.getSlotElementId();console.log(id+"is empty");//var element = document.getElementById(target);
//  collapse(element, slider);
}});googletag.pubads().enableSingleRequest();googletag.enableServices();googletag.display('div-gpt-ad-'+id+'-0');//googletag.pubads().refresh([slot]);
},timeout);}}/* * * END * * */ /* ADDITIONAL FUNCTIONS, PAGE, IMAGE ETC */ /* currency letter*/function getCurrencyLetter(currency){return currencies[currency]||currency;}/* page title */function getPageTitle(){var title;if(window.sarafanSDK.site==='cosmo'){var articles=document.querySelectorAll('.article-title');title=articles[articles.length-1].value.trim()||'';}else{if(document.getElementsByTagName("title").length>0){title=document.getElementsByTagName("title")[0].innerHTML.trim()||'';}else{title='';}}return title.trim().replace(/\r?\n/g," ");}/* page header */function getHeader(el){var header=null;var h1=el.getElementsByTagName('h1');var h2=el.getElementsByTagName('h2');if(h2.length>0){header=h2[h2.length-1];}if(h1.length>0){header=h1[h1.length-1];}return header;}function getPageHeader(){var header=null;if(_border){var areasPack=_border.replace(" ","").split(",");//TODO exclude :not and others
...
var scrolled=window.pageYOffset||document.documentElement.scrollTop;var floorY=nextFloor.getBoundingClientRect().top+scrolled;if(scrolled+document.documentElement.clientHeight+nextFloorOffset>=floorY){populateNextFloor();// nextFloor = getNextFloor();
// populateNextFloor();
...
var YApixel=document.createElement('img');YApixel.src='https://mc.yandex.ru/watch/45068210';YApixel.classList.add('sarafan-pixel');document.body.appendChild(YApixel);}function hideAllPanels(){document.querySelectorAll("."+sarafanOffersPanelClass).forEach(function(v){v.style.display='none';});}/* * * END * * */SarafanParent.prototype.restart=function(){if(test){console.log('Sarafan widget restart');console.log('new release!');}window.sarafanSDK.options(version).then(function(){rewriteOptionsFromApiResponse();if(_border){var sections=document.querySelectorAll(_border);area=sections;}processNewImages();if(_enableFrames&&_enableFloorMode){processNewFrames();}if(observer){observer.observe(targetNode,observerConfig);}});//     .catch(
//     function (reason) {
//         if (test) console.log('Sarafan widget error: ' + reason);
//         rewriteOptionsFromApiResponse();
//     }
// );
};SarafanParent.prototype.stop=function(){if(observer){observer.disconnect();}};function isParentsAreVisible(el){if(el.parentNode){if(el.parentNode.tagName==="BODY"){return false;}if(el.parentNode.style.opacity==="0"||el.parentNode.style.visibility==="hidden"){return true;}else{return isParentsAreVisible(el.parentNode);}}}checkViewPanelsVisibility=function checkViewPanelsVisibility(){var viewPanels=document.body.getElementsByClassName("sarafan-view");for(var i=0;i<viewPanels.length;i++){checkViewPanelVisibility(viewPanels[i]);}};function checkViewPanelVisibility(viewPanel){if(isParentsAreVisible(viewPanel)){// viewPanel.style.height = "auto";
...
window.Sarafan.prototype.constructor=window.Sarafan;var Slider=/*#__PURE__*/function(){function Slider(sarafanOffers,mode,widgetType,dataForStat,btn,image){var _this=this;_classCallCheck(this,Slider);this.mode=mode;this.container=sarafanOffers;this.startX=0;this.prev;this.dataForStat=dataForStat;this.next;this.btn=btn;this.distance=0;this.delta=0;this.count=0;this.offset=0;this.minOffeset=0;this.offers=[];this.offersData=[];this.dblClickQueue=[];mode===MULTIPLE_MODE?this.slideWidth=130:this.slideWidth=270;this.margin=10;this.newSlider=document.createElement("sfandiv");this.newSliderInner=document.createElement("sfandiv");this.newSliderContent=document.createElement("sfandiv");this.newSlider.className='new-sarafan-slider';this.newSliderContent.className='new-sarafan-slider__content';this.newSliderInner.className='new-sarafan-slider__inner';this.newSliderContent.appendChild(this.newSliderInner);this.newSlider.appendChild(this.newSliderContent);this.PREV="prev";this.lastDisplayedOffer=0;this.NEXT="next";this.image=image;this.url=location.href;this.widgetType=widgetType;//this.newSliderInner.addEventListener("touchstart", (e) => this.handleStart(e), false);
//this.newSliderInner.addEventListener("touchend", (e) => this.handleEnd(e), false);
//this.newSliderInner.addEventListener("touchend", (e) => this.handleEnd(e), false);
//this.newSliderInner.addEventListener("touchcancel", (e) => this.handleCancel(e), false);
//this.newSliderInner.addEventListener("touchmove", (e) => this.handleMove(e), false);
if(isMobile()){this.newSliderInner.addEventListener("scroll",function(e){return _this.scroll(e);},false);}window.addEventListener("resize",function(){_this.update();});this.container.appendChild(this.newSlider);this.addControls();//setTimeout(() => this.collapse(this.offers), 2000);
//this.prev.addEventListener ('DOMNodeInserted', ()=>this.update(), false);
}_createClass(Slider,[{key:"collapse",value:function collapse(){if(this.btn){setOffersPanelWidthByOffersWidth(this.container.parentNode,this.offers);calcOffersPanelPos(this.btn,this.container.parentNode,this.image);}this.update();}},{key:"scroll",value:function scroll(){var _this2=this;clearTimeout(this.timer);this.timer=setTimeout(function(){return _this2.sendStatistic();},500);}},{key:"getSliderWidth",value:function getSliderWidth(){return this.newSlider.parentElement.offsetWidth;}},{key:"getSliderInnerWidth",value:function getSliderInnerWidth(){return this.newSliderInner.offsetWidth;}},{key:"update",value:function update(){//this.newSliderInner.style.width = this.offersWidth + "px";
if(isMobile()){this.newSliderInner.style.width="100%";this.newSliderInner.style.overflowX="auto";}else{this.newSliderInner.style.width="auto";this.newSliderInner.style.overflowX="hidden";var sw=this.getSliderWidth();var iw=this.getSliderInnerWidth();if(sw!==0&&iw!==0){if(sw>=iw){this.disableContols();}else{this.enableContols();}}}}},{key:"disableContols",value:function disableContols(){this.prev.style.display="none";this.next.style.display="none";}},{key:"enableContols",value:function enableContols(){var maxOffeset=-(this.getSliderInnerWidth()-this.getSliderWidth());if(this.offset<=maxOffeset){this.prev.style.display="flex";this.next.style.display="none";}else if(this.offset>=this.minOffeset){this.prev.style.display="none";this.next.style.display="flex";}else{this.prev.style.display="flex";this.next.style.display="flex";}}},{key:"renderOffers",value:function renderOffers(offersData){var _this3=this;var offerNodes=[];var offerClass="";if(this.mode===MULTIPLE_MODE){offerClass='multiple-offer';}if(this.mode===SINGLE_MODE){offerClass='single-offer';}var _loop2=function _loop2(i){var offerADV=null;// NODE object of offer for statistics working
switch(_this3.offersData[i].type){case'adfox':{var _getAdfoxCodes3=getAdfoxCodes(offersData[i]),floorDiv=_getAdfoxCodes3.floorDiv,floorJs=_getAdfoxCodes3.floorJs;offerADV=document.createElement('sfandiv');offerADV.classList.add('offer',"sf-adfox",'sf-ad',offerClass);offerADV.innerHTML+=floorDiv;offerNodes.push(offerADV);if(_this3.mode===SINGLE_MODE){offerADV.style.width="270";offerADV.style.height="170px";}else{offerADV.style.width="130px";if(window.Cart){offerADV.style.height="300px";}else{offerADV.style.height="270px";}}setTimeout(function(){new Function(floorJs)();if(test)console.log('Sarafan adfox loaded '+floorDiv);},100);window.floorCount++;break;}case'adfox_p':{var _getAdfoxCodes4=getAdfoxCodes(offersData[i]),_floorDiv3=_getAdfoxCodes4.floorDiv,_floorJs3=_getAdfoxCodes4.floorJs;offerADV=document.createElement('sfandiv');offerADV.classList.add('sf-promo',"offer",'sf-ad',"sf-adfox");offerADV.style.width=_this3.container.parentNode.offsetWidth-20+"px";offerADV.style.maxWidth="550px";offerADV.style.minWidth="270px";if(window.Cart){offerADV.style.height="300px";}else{offerADV.style.height="270px";}offerADV.innerHTML+=_floorDiv3;offerNodes.push(offerADV);setTimeout(function(){new Function("onError",_floorJs3)(function(error){return alert(JSON.stringify(_floorDiv3,error));});},100);break;}case'adfox_mm':{var _getAdfoxCodes5=getAdfoxCodes(offersData[i]),_floorDiv4=_getAdfoxCodes5.floorDiv,_floorJs4=_getAdfoxCodes5.floorJs;offerADV=document.createElement('sfandiv');offerADV.classList.add('offer',"sf-adfox",'sf-mm','sf-ad',offerClass);offerADV.innerHTML+=_floorDiv4;if(_this3.mode===SINGLE_MODE){offerADV.style.width="270";offerADV.style.height="170px";}else{offerADV.style.width="130px";if(window.Cart){offerADV.style.height="300px";}else{offerADV.style.height="270px";}}offerNodes.push(offerADV);setTimeout(function(){new Function(_floorJs4)();},100);break;}case'doubleclick':{var dblObj=getDoubleClickDiv2(offersData[i],_this3.mode);offerNodes.push(dblObj.div);//container.appendChild(dblObj.div);
_this3.dblClickQueue.push(dblObj);break;}default:offerNodes.push(getSarafanOffer(offersData[i],_this3.mode,null,_this3.dataForStat));break;}if(offerADV){var clickListener=function clickListener(event){event.preventDefault();var mousePosition=[event.clientX,event.clientY];sarafanSDK.sendEvent(_objectSpread({type:"click.offer"},this.dataForStat,{offersData:[offersData[i]],component:offerADV,mousePosition:mousePosition}));};offerADV.addEventListener('click',clickListener,false);offerADV.addEventListener('auxclick',clickListener,false);offerADV.addEventListener('contextmenu',function(e){e.preventDefault();});}};for(var i=0;i<offersData.length;i++){_loop2(i);}if(offerNodes.length>0&&!this.hiddenOffer){this.hiddenOffer=offerNodes[0].cloneNode(true);this.hiddenOffer.classList.add("hidden-offer");this.newSliderContent.appendChild(this.hiddenOffer);this.hiddenOffer.style.visibility="hidden";}offerNodes.map(function(o){_this3.newSliderInner.appendChild(o);});this.offers=this.offers.concat(offerNodes);if(this.widgetType===WIDGET_TYPE_FLOOR){if(test){debugger;}}if(_enableDblClick){setTimeout(function(){_this3.dblClickQueue.forEach(function(v){dblClickRenderSlot2(v.div,v.str,v.resolutions,v.id,_this3);});_this3.dblClickQueue=[];},200);}}},{key:"appendOffers",value:function appendOffers(offersData){var _this4=this;var tmp=offersData.map(function(o){o.showed=false;return o;});this.offersData=this.offersData.concat(tmp);this.renderOffers(tmp);setTimeout(function(){_this4.update();_this4.sendFirstShowStatistic();},100);}},{key:"sendFirstShowStatistic",value:function sendFirstShowStatistic(){var _this5=this;var offersData=this.getShowedOffers();if(offersData.length>0){var offersPanel=this.container.parentNode;var selectorStr='.'+offersPanel.classList[0];if(offersPanel.hasAttribute('data-sarafan-src')){selectorStr+='['+'data-sarafan-src="'+offersPanel.getAttribute('data-sarafan-src')+'"]';}if(offersPanel.hasAttribute('data-floor-id')){selectorStr+='['+'data-floor-id="'+offersPanel.getAttribute('data-floor-id')+'"]';}selectorStr+=' .sarafan-offers';// first viewport view mode showing
inView(selectorStr).once('enter',function(){sarafanSDK.sendEvent(_objectSpread({},_this5.dataForStat,{offersData:offersData,type:"viewport.offers"}));});}}},{key:"getShowedOffers",value:function getShowedOffers(){var showedOffers=[];var last=this.getLastDiplayedOfferIndex();for(var i=this.lastDisplayedOffer;i<=last;i++){if(this.offersData[i]&&this.offersData[i].showed===false){var o=_objectSpread({},this.offersData[i]);delete o["showed"];showedOffers.push(o);this.offersData[i].showed=true;}}this.lastDisplayedOffer=last;return showedOffers;}},{key:"sendStatistic",value:function sendStatistic(){var offersData=this.getShowedOffers();if(offersData.length>0){sarafanSDK.sendEvent(_objectSpread({},this.dataForStat,{offersData:offersData,type:"viewport.offers"}));}}// setFirstDiplayedOfferIndex() {
//     for (let i = 0; i < this.offers.length; i++) {
//         const left = this.offers[i].offsetLeft;
//         let d = Math.abs(this.offset);
//         if (d <= left) {
//             this.firstDiplayedOfferIndex = i;
//             break;
//         }
//     }
// }
},{key:"getLastDiplayedOfferIndex",value:function getLastDiplayedOfferIndex(){//let l = this.offers.map(o => o.offsetLeft)
var last;var offset=isMobile()?this.newSliderInner.scrollLeft:this.offset;for(var i=this.lastDisplayedOffer;i<this.offers.length;i++){var left=this.offers[i].offsetLeft+this.offers[i].offsetWidth;var d=Math.abs(offset)+this.getSliderWidth();if(d<=left){last=i;break;}if(!last){last=this.offers.length-1;}}return last;}//
// handleStart(e) {
//     // this.startX = e.changedTouches[0].clientX;
//     // this.tmpOffest = this.offset;
//     // this.setFirstDiplayedOfferIndex();
//     console.log("start");
// }
//
// handleMove(e) {
//     // this.delta = this.startX - e.changedTouches[0].clientX;
//     // this.offset = this.tmpOffest - this.delta;
//     // this.translate();
// }
//
// handleEnd(e) {
//     console.log("end");
//     // this.setLastDiplayedOfferIndex();
//     // this.sendStatistic();
// }
},{key:"addControls",value:function addControls(){var _this6=this;this.prev=document.createElement('sfandiv'),this.next=document.createElement('sfandiv');this.prev.className='new-sarafan-slider__prev';this.prev.innerHTML='<svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">\n'+'<path d="M14.7912 7.00502H3.62125L8.50125 2.12502C8.89125 1.73502 8.89125 1.09502 8.50125 0.705022C8.31442 0.51777 8.06077 0.412537 7.79625 0.412537C7.53173 0.412537 7.27808 0.51777 7.09125 0.705022L0.501248 7.29502C0.111248 7.68502 0.111248 8.31502 0.501248 8.70502L7.09125 15.295C7.48125 15.685 8.11125 15.685 8.50125 15.295C8.89125 14.905 8.89125 14.275 8.50125 13.885L3.62125 9.00502H14.7912C15.3412 9.00502 15.7912 8.55502 15.7912 8.00502C15.7912 7.45502 15.3412 7.00502 14.7912 7.00502Z" fill="#9E9E9E"/>\n'+'</svg>\n';this.next.className='new-sarafan-slider__next';this.next.innerHTML='<svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">\n'+'<path d="M1.20874 9.00002H12.3787L7.49874 13.88C7.10874 14.27 7.10874 14.91 7.49874 15.3C7.88874 15.69 8.51874 15.69 8.90874 15.3L15.4987 8.71002C15.8887 8.32002 15.8887 7.69002 15.4987 7.30002L8.91874 0.700017C8.73191 0.512765 8.47826 0.407532 8.21374 0.407532C7.94922 0.407532 7.69557 0.512765 7.50874 0.700017C7.11874 1.09002 7.11874 1.72002 7.50874 2.11002L12.3787 7.00002H1.20874C0.65874 7.00002 0.20874 7.45002 0.20874 8.00002C0.20874 8.55002 0.65874 9.00002 1.20874 9.00002Z" fill="#9E9E9E"/>\n'+'</svg>\n';if(this.mode===SINGLE_MODE){this.prev.classList.add("new-sarafan-slider__prev_single");this.next.classList.add("new-sarafan-slider__next_single");}else{this.prev.classList.add("new-sarafan-slider__prev_multiple");this.next.classList.add("new-sarafan-slider__next_multiple");}this.newSlider.appendChild(this.prev);this.newSlider.appendChild(this.next);this.prev.addEventListener('click',function(e){_this6.moveSlides(_this6.PREV);});this.next.addEventListener('click',function(e){_this6.moveSlides(_this6.NEXT);});}},{key:"binarySearch",value:function binarySearch(t,direction)// t - искомый элемент,
{// A - упорядоченный массив, в котором ищем.
var i=0,j=this.offers.length-1,k;var l=this.offers.map(function(o){return o.offsetLeft;});var w=this.offers.map(function(o){return o.offsetLeft+o.offsetWidth;});var o1,o2;var iter=0;while(i<j){k=Math.floor((i+j)/2);if(direction===this.PREV){o1=this.offers[k].offsetLeft;o2=this.offers[k-1]?this.offers[k-1].offsetLeft:this.margin;if(t<=o1)j=k;else i=k+1;// if (t <= o1 && t >= o2) {
//     console.log(iter);
//     return k;
// }
}else{o1=this.offers[k].offsetLeft+this.offers[k].offsetWidth;o2=this.offers[k-1]?this.offers[k-1].offsetLeft+this.offers[k-1].offsetWidth+this.margin:this.margin+this.offers[k].offsetWidth;if(t<=o1)j=k;else i=k+1;// if (t <= o1 && t >= o2) {
//     console.log(iter);
//     return k;
// }
}iter++;}//console.log(iter);
// if (this.offers[ i ] === t) return i;     // На выходе индекс искомого элемента.
return i;// Если искомого элемента нет в массиве, то -1.
}},{key:"moveSlides",value:function moveSlides(direction){var _this7=this;var sliderWidth=this.getSliderWidth();this.newSliderInner.classList.add('new-sarafan-slider__inner_animate');// this.calcDistance();
//let l = this.offers.map(o => o.offsetLeft)
var iter=0;if(direction===this.PREV){var d=Math.abs(this.offset);var index=this.binarySearch(d,this.PREV);this.offset=-(this.offers[index].offsetLeft-sliderWidth);// for (let i = this.offers.length - 1; i >= 0; i--) {
//     const left = this.offers[i].offsetLeft;
//     let d = Math.abs(this.offset);
//     if (d < this.margin) {
//         d = this.margin;
...
//     if (d >= left) {
//         let pos;
//         pos = i + 1 > this.offers.length - 1 ? pos = this.offers.length - 1 : pos = i + 1;
//         this.offset = -(this.offers[pos].offsetLeft - sliderWidth);
//         console.log(iter);
//         break;
//     }
//     iter++;
// }
}else if(direction===this.NEXT){var _d=Math.abs(this.offset)+sliderWidth;var _index=this.binarySearch(_d,this.NEXT);this.offset=-(this.offers[_index].offsetLeft-this.margin);// for (let i = 0; i < this.offers.length; i++) {
//     const left = this.offers[i].offsetLeft + this.offers[i].offsetWidth;
//     let d = Math.abs(this.offset) + sliderWidth;
//     if (d <= left) {
//         let pos;
//         this.offset = -(this.offers[i].offsetLeft - this.margin);
//         console.log(iter);
//         break;
//     }
//     iter++;
// }
}if(direction===this.NEXT){this.sendStatistic();}this.translate();setTimeout(function(){_this7.stopAnimation();_this7.update();},500);}},{key:"translate",value:function translate(){var maxOffeset=-(this.getSliderInnerWidth()-this.getSliderWidth());if(this.offset<=maxOffeset){this.offset=maxOffeset;}if(this.offset>=this.minOffeset){this.offset=this.minOffeset;}this.newSliderInner.style.left=this.offset+"px";}},{key:"stopAnimation",value:function stopAnimation(){this.newSliderInner.classList.remove('new-sarafan-slider__inner_animate');}}]);return Slider;}();;function setOffersPanelWidthByOffersCount(offers,totalArr){switch(true){case totalArr.length===1:offers.classList.add("sf-1");break;case totalArr.length===2:offers.classList.add("sf-2");break;case totalArr.length===3:offers.classList.add("sf-3");break;case totalArr.length===4:offers.classList.add("sf-4");break;case totalArr.length>4:offers.classList.add("sf-4-plus");break;}}function setOffersPanelWidthByOffersWidth(offers,offerNodes){var count=0;offerNodes.map(function(o){if(o.offsetWidth>0){count++;}});switch(true){case count===1:offers.classList.remove("sf-1","sf-2","sf-3","sf-4","sf-4-plus");offers.classList.add("sf-1");break;case count===2:offers.classList.remove("sf-1","sf-2","sf-3","sf-4","sf-4-plus");offers.classList.add("sf-2");break;case count===3:offers.classList.remove("sf-1","sf-2","sf-3","sf-4","sf-4-plus");offers.classList.add("sf-3");break;case count===4:offers.classList.remove("sf-1","sf-2","sf-3","sf-4","sf-4-plus");offers.classList.add("sf-4");break;case count>=5:offers.classList.remove("sf-1","sf-2","sf-3","sf-4","sf-4-plus");offers.classList.add("sf-4-plus");break;}}function calcOffersPanelPos(btn,offersPanel,image){var btnCoordinates=getCoords(btn);var clientWidth=window.innerWidth||document.documentElement.clientWidth;var clientHeight=window.innerHeight||document.documentElement.clientHeight;var btnRect=btn.getBoundingClientRect();var btnWidth=btnRect.right-btnRect.left,btnHeight=btnRect.bottom-btnRect.top;var offersRect=offersPanel.getBoundingClientRect();var offersWidth=offersRect.right-offersRect.left,offersHeight=offersRect.bottom-offersRect.top;var fixLeft=0,fixTop=0,addTop=0;// special for Eva
...
offersPanel.style.left=btnCoordinates.left+'px';offersPanel.style.top=ATC+'px';}else if(_corner==='topright'){offersPanel.style.left=btnCoordinates.left+btn.offsetWidth-offersPanel.offsetWidth+'px';offersPanel.style.top=ATC+'px';}else if(_corner==='bottomleft'){// coords for dropping down (offers showing to down)
offersPanel.style.left=btnCoordinates.left+fixLeft+'px';offersPanel.style.top=ATC-fixTop-addTop+'px';}else if(_corner==='bottomright'){// coords for dropping down (offers showing to down)
offersPanel.style.left=btnCoordinates.left+btn.offsetWidth-offersPanel.offsetWidth+fixLeft+'px';offersPanel.style.top=ATC-fixTop-addTop+'px';}else if(_corner==='bottomcenter'){offersPanel.style.left=btnCoordinates.left-offersWidth/2+btnWidth/2+fixLeft+'px';offersPanel.style.top=ATC-fixTop-addTop+'px';}else if(_corner==='marginbottom'){offersPanel.style.left=btnCoordinates.left-offersWidth/2+btnWidth/2+'px';offersPanel.style.top=btnCoordinates.top+btnHeight+offersShift+'px';}if(btnCoordinates.left<0){offersPanel.style.left=0;}if(btnCoordinates.top<0){offersPanel.style.top=0;}if(location.host=="www.wday.ru"&&image.matches(".swiper-zoom-container img")){offers.style.top=Math.abs(parseInt(document.body.style.top))+parseInt(offers.style.top)+"px";offers.style.zIndex=99999;}else{checkElementPositionForViewport(offers,clientWidth,clientHeight);}}function getOfferWithDblClick(offersArr){var cnt=getOffersCnt2(document.documentElement.clientWidth);var totalArr=[];var item={type:'doubleclick',place:'ad'};if(offersArr.length===1){totalArr.push(offersArr[0]);totalArr.push(item);}else{var promo=Object.assign({},item);promo.place='promo';promo.size=Math.min(cnt,offersArr.length);var b=0;var promoIsAppend=false;for(var o=0;o<offersArr.length;o++){totalArr.push(offersArr[o]);if((o+1)%(cnt-1)===0&&b<2){totalArr.push(item);b++;if(promo&&!promoIsAppend){totalArr.push(promo);promoIsAppend=true;}}}if(totalArr.length<cnt&&b===0){totalArr.push(item);}if(promo&&!promoIsAppend){totalArr.push(promo);}}return totalArr;}function getOffersWithAdfox(offersArr,adOffersArr){/* get complete the look cards */var completes=adOffersArr.filter(function(e){return e.type==='A';});/* get micromedia cards */var micromedias=adOffersArr.filter(function(e){return e.type==='adfox_mm';});/* get adfox cards */var ads=adOffersArr.filter(function(e){return!completes.includes(e);}).filter(function(e){return e.type==='adfox';}).filter(function(e){return e.insert_code_div.length>5;});/* get promo cards */var promos=adOffersArr.filter(function(e){return e.type==='adfox_p';});/* get offers count for current resolution */ /* Merge arrays with offers, ADs and complete the look's at the end */micromedias=micromedias.concat(ads);return getCollectedOffersWithAdfox(offersArr,micromedias,promos[0]);}function getCollectedOffersWithAdfox(offersArr,micromedias,promo){var cnt=getOffersCnt2(document.documentElement.clientWidth);var totalArr=[];if(offersArr.length===1){totalArr.push(offersArr[0]);if(micromedias[0]){totalArr.push(micromedias[0]);}}else{var b=0;var promoIsAppend=false;for(var o=0;o<offersArr.length;o++){totalArr.push(offersArr[o]);if((o+1)%(cnt-1)===0&&b<2&&micromedias[b]){totalArr.push(micromedias[b]);b++;if(promo&&!promoIsAppend){totalArr.push(promo);promoIsAppend=true;}}}if(totalArr.length<cnt&&b===0&&micromedias[b]){totalArr.push(micromedias[b]);}if(promo&&!promoIsAppend){totalArr.push(promo);}}return totalArr;}function getDoubleClickDiv2(e,mode,i){var id=Math.random().toString().slice(5);var div=document.createElement('sfandiv');//div.style = "width:500px; height: 500px;"
var str=getPlacement(e,i);var width,height;var resolutions=[];div.classList.add('offer','sf-ad','sf-dbl');if(mode===MULTIPLE_MODE){if(window.Cart){height=300;}else{height=270;}width=130;resolutions=[[width,height]];if(e.place==="promo"){if(e.size===2){// div.style = `width:270; height: ${height}px;`
//div.style = `width:100%`
resolutions=[270,height];}if(e.size===3){resolutions=[[410,height],[270,height]];//div.style = `width:100%`
//  div.style = `width:410; height: ${height}px;`
// resolutions = [[410, height]];
}if(e.size>=4){resolutions=[[270,height],[410,height],[550,height]];// div.style = `width:100%`
// div.style = `width:550; height: ${height}px;`
//resolutions = [[550, height]];
}//resolutions = [[550, 280], [410, 280], [280, 280]];
}}if(mode===SINGLE_MODE){resolutions=[[270,170]];}div.insertAdjacentHTML('afterbegin','<div id=\'div-gpt-ad-'+id+'-0\'></div>');return{div:div,str:str,resolutions:resolutions,id:id};}function dblClickRenderSlot2(div,str,resolutions,id,slider){if(str&&resolutions&&id){var googletag=window.googletag;var slot=googletag.defineSlot(str,resolutions,'div-gpt-ad-'+id+'-0').addService(googletag.pubads());googletag.pubads().collapseEmptyDivs(true,true);// googletag.pubads().addEventListener('SlotRequested', function(event) {
//     console.log();
// });
...
//     console.log();
// });
googletag.pubads().addEventListener('slotRenderEnded',function(event){if(event.slot===slot){if(event.isEmpty){div.style.display="none";slider.update();}else{//slider.update();
slider.update();}}});// googletag.pubads().addEventListener('impressionViewable', function(event) {
//     console.log();
// });
googletag.pubads().enableSingleRequest();googletag.enableServices();googletag.display('div-gpt-ad-'+id+'-0');googletag.pubads().refresh([slot]);// window.googletag = window.googletag || {cmd: []};
// googletag.cmd.push(function() {
//     googletag.defineSlot('81006599/hmiru-wday/sarafan"', resolutions, 'div-gpt-ad-' + id + '-0').addService(googletag.pubads());
//     googletag.pubads().enableSingleRequest();
//     googletag.enableServices();
//googletag.display('div-gpt-ad-' + id + '-0');
//});
}}function getPlacement(e,i){var str,placement;if(location.host==="www.wday.ru"||location.host==="www.woman.ru"){str=_dblClickCampaign+'/sarafan-ad-1';}else{if(e.place==="promo"){str=_dblClickCampaign+"/sft-promo";}else{str=_dblClickCampaign+"/sft-mm";}}return str;}function getOffersCnt2(w){var c=5;if(w<429){c=2;}else if(w<569){c=3;}else if(w<619){c=4;}return c;}/* generate floor block */function getRetailer(){if(location.href==="https://sarafan.tech/blog/the-best-smartphones-for-2019"){return"amazon.com";}else{return"";}}})();
